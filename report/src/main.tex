...
After extensive manual iteration the best performing model (\textbf{v9}) has the following linguistic variables and terms illustrated in the following~\cref{fig:bellv9_system_load,fig:bellv9_latency,fig:bellv9_clp}.
...
Using the \texttt{bell\_v9} model as a base, including its linguistic variables and rule base, we conducted a thorough hyperparameter tuning of the parameters $a$, $b$, and $c$ of the membership functions.  
...
This corresponds to a six-fold improvement over the manually obtained \emph{MSE} of $0.04473$ for the \texttt{mamdani\_bell\_v9}.
...
We evaluated the system using the 10 data points provided, this model achieved the lowest Mean Squared Error (MSE) of 0.00735.
...
\caption{\emph{MSE} of the top 3 performing models (\texttt{bellv9}, \texttt{bell\_hyper}, and \texttt{trianglev4})}
\label{fig:mse_3best}
\end{figure}
...
\caption{\emph{MSE} of the top 2 performing models (\texttt{bell\_hyper}, and \texttt{trianglev4})}
\label{fig:mse_2best}
\end{figure}
...
However, the membership functions are drastically different (see \cref{fig:bell_hyper_system_load,fig:bell_hyper_latency,fig:bell_hyper_clp} for \texttt{bell\_hyper} and \cref{fig:systemload_triangular_v4,fig:clp_triangular_v4,fig:latency_triangular_v4} for \texttt{trianglev4}).
