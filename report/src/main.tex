...
\begin{itemize}
    ...
    \item Fuzzy System Classification Label (NN ground truth)
...
The best possible outcome would be for the NN to perform exactly like the FS. \\
If we ``normalize'' the NN MSE as such: $\overline{\text{MSE}}_{\text{NN}}=\text{MSE}_{\text{NN}} - \text{MSE}_{\text{FS}}$
we obtain $\overline{MSE}_{\text{NN}}=0.02$ which is very close to the test loss obtained ($0.0279$).
