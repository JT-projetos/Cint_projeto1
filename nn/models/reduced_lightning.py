...
if __name__ == '__main__':
    ...
    # Step 2: Define data
    # -------------------
    df = pd.read_csv('../../gen_input/trianglev4_uniform10000.csv')
    df = df.drop(["V_MemoryUsage",'V_ProcessorLoad', 'V_InpNetThroughput', 'V_OutNetThroughput', 'V_OutBandwidth', 'V_Latency' ], axis='columns')
    ...
    logger = TensorBoardLogger('../model_logs', name='reduced_model')
    checkpoint = ModelCheckpoint(monitor='val_loss', mode='min', filename='reduced-{epoch:02d}-{val_loss:.2f}')
